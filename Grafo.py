...
class Graph:
    ...
    def search_other(self, vertice_value, key):
        vertice = self.search(vertice_value)
        if vertice is not None:
            if 'other_value' in self.elements[vertice]:
                if key:
                    return self.elements[vertice]['other_value'][key]
                else:
                    return self.elements[vertice]['other_value']
    ...
    def insert_arista(self, origen, destino, peso):
        """Agrega una arista del vertice origen al destino"""
    ...
    def lista_arista (self):
        lista=[]
        for i in range(len(self.elements)-1):
            for j in range(i+1, len(self.elements)):
                pos, k=self.search_arista(self.elements[i]['value'], self.elements[j]['value'])
                arista = self.elements[pos]['aristas'][k]
                lista.append({'origen': self.elements[i]['value'], 'destino': self.elements[j]['value'], 'peso':self.elements[pos]['aristas'][k]['peso']})
        return lista
    
    def delete_arista(self, origen, destino):
        """Elimina y devuelve arista segun origen/destino"""
    ...
    def delete_vertice(self, value):
        """Elimina y devuelve vertice segun valor"""
    ...
    def exist_path(self, origen, destino):
        """Devuelve si es posible ir de un vertice origen hasta un destino"""
    ...
    def dijkstra(self, origen):
        """Camino mas corto desde el vertice origen al destino"""
            ...
            for adjacente in adjacentes:
                ...
                if pos is not None:
                    if costo_nodo_actual + adjacente['peso'] < no_visitados.elements[pos][0]:
                        ...
                        no_visitados.change_priority(pos, costo_nodo_actual + adjacente['peso'])
        return camino

    def kruskal(self, origen):
        """Devuelve arbol de expansion minimo a partir del vertice de inicio"""
