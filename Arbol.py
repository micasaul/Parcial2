...
class BinaryTree:
    ...
    def insert_node(self, value, other_value=None):
        ...
        def __insert(root, value, other_value=None):
            if root is None:
                return BinaryTree.___Node(value, other_value=other_value)
    ...
    def search_lista(self, key):
        """Busca un nodo con valor coincidente y crea una lista"""
        def __search_lista(root, key):
            def value_s(value):
                if "/" in value:
                    values=value.split("/")
                    return values
                    ...
                    return value
            if root is not None:
                __search_lista(root.left, key)
                roots= value_s(root.value)
                if type(roots) == list:
                    for i in roots:
                        if i.startswith(key):
                            lista.append(root.other_value['nombre'])
                else:
                    if roots.startswith(key):
                        lista.append(root.other_value['nombre'])
                __search_lista(root.right, key)

        lista = []
        __search_lista(self.root, key)
        return lista
    ...
    def inorden_values(self):
        ...
        def __inorden(root):
            if root is not None:
                __inorden(root.left)
                print(f"{root.value}, otro: {root.other_value}")
                __inorden(root.right)

        if self.root is not None:
            __inorden(self.root)

    def inorden_nivel(self):
        """Recorre inorden devuelve con nivel"""
        def __inorden(root):
            if root is not None:
                __inorden(root.left)
                lista.append({'nombre': root.value, 'nivel': root.other_value['nivel']})
                __inorden(root.right)

        lista=[]
        if self.root is not None:
            __inorden(self.root)
        lista.sort(key=lambda x: (x["nivel"], x["nombre"]))
        return lista
